<x-officer-layout>
    ...
    @if ($events->isNotEmpty())
        {{-- Tampilkan Interface Scanner Jika Ada Event Yang Ditugaskan --}}
            ...
            <div x-show="result" x-transition class="rounded-2xl border-2 p-5"
                ...
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
                        :class="{ 'bg-emerald-500': result?.status === 'valid', 'bg-red-500': result
                            ?.status === 'invalid', 'bg-amber-500': result?.status === 'already_used' }">
                        <i class="ti text-white text-2xl"
                            :class="{ 'ti-circle-check': result?.status === 'valid', 'ti-circle-x': result
                                    ?.status === 'invalid', 'ti-clock': result?.status === 'already_used' }"></i>
                    </div>
                    <div>
                        <p class="text-base font-bold"
                            :class="{ 'text-emerald-700': result?.status === 'valid', 'text-red-700': result
                                    ?.status === 'invalid', 'text-amber-700': result?.status === 'already_used' }"
                            x-text="result?.status === 'valid' ? 'TIKET VALID ✓' : (result?.status === 'already_used' ? 'SUDAH DIGUNAKAN' : 'TIKET TIDAK VALID')">
                        </p>
                        <p class="text-sm mt-0.5"
                            :class="{ 'text-emerald-600': result?.status === 'valid', 'text-red-600': result
                                    ?.status === 'invalid', 'text-amber-600': result?.status === 'already_used' }"
                ...
                <template x-if="result?.holder">
                    <div class="space-y-2 border-t pt-3"
                        :class="{ 'border-emerald-200': result?.status === 'valid', 'border-amber-200': result
                                ?.status === 'already_used' }">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium text-gray-700 text-right max-w-[60%]"
                                x-text="result?.event"></span>
                        </div>
                        <div class="flex justify-between text-sm">
    ...
    @else
        {{-- Empty State Jika Tidak Ada Event Aktif / Ditugaskan Untuk Officer Ini --}}
        <div class="bg-white border border-gray-100 rounded-2xl p-8 text-center my-6">
            <div
                class="w-12 h-12 bg-amber-50 text-amber-500 rounded-xl flex items-center justify-center mx-auto mb-3.5">
                <i class="ti ti-scan-off text-2xl"></i>
            </div>
            <p class="text-sm font-semibold text-gray-950">Tidak Ada Tugas Scan</p>
            <p class="text-xs text-gray-400 mt-1 max-w-xs mx-auto leading-relaxed">
                Saat ini Anda tidak memiliki event aktif yang ditugaskan atau seluruh event yang terhubung telah dihapus
                oleh administrator.
            </p>
        </div>
    @endif
    ...
    @push('scripts')
        ...
        <script>
            // Skrip JavaScript dibungkus pengkondisian parsial via fungsi helper agar jika kosong tidak memicu error JS
