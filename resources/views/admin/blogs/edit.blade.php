    ...
    <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="excerpt" rows="2" maxlength="500"
                            class="w-full px-3 py-2.5 border border-gray-100 rounded-xl text-sm text-gray-700 outline-none focus:border-gray-300 focus:ring-0 bg-white resize-none @error('excerpt') @enderror">{{ old('excerpt', $blog->excerpt) }}</textarea>
                        @error('excerpt')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="rounded-xl overflow-hidden">
                            <textarea name="content" id="blog-editor">{{ old('content', $blog->content) }}</textarea>
                        </div>
                        @error('content')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div class="space-y-5">
                ...
                {{-- Thumbnail dengan fallback gambar lama --}}
                <div class="bg-white rounded-2xl border border-gray-100 p-5" x-data="{
                    preview: '{{ $blog->thumbnail ? Storage::url($blog->thumbnail) : '' }}',
                    showPreview(event) {
                ...
                <div class="space-y-2">
                    <button type="submit"
                        ...
                        <i class="ti ti-check text-base"></i> Update Artikel
                    </button>
                    <a href="{{ route('admin.blogs.show', $blog) }}"
