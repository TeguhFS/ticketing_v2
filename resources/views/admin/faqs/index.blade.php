                            ...
                            {{ $faq->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-400' }}">
                                    ...
                                    <div class="flex items-center gap-1.5 flex-shrink-0">
                                        ...
                                        <button
                                            @click="
        deleteFaqId = {{ $faq->id }};
        deleteFaqQuestion = '{{ addslashes($faq->question) }}';
    "
                                            type="button"
                                ...
                                {{-- Answer Preview --}}
                                <div x-cloak x-show="expanded" x-transition:enter="transition ease-out duration-150"
                                ...
                                {{-- Edit Form --}}
                                <div x-cloak x-show="editOpen" x-transition class="border-t border-gray-50">
        ...
        {{-- Delete Modal --}}
        <div x-show="deleteFaqId" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
            style="display:none">

            {{-- Backdrop --}}
            <div class="absolute inset-0 bg-black/50" @click="deleteFaqId = null">
            </div>

            {{-- Modal --}}
            <div x-show="deleteFaqId" x-transition
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-1">
                            FAQ berikut akan dihapus permanen:
                        </p>

                        <div class="mt-3 p-3 rounded-xl bg-gray-50 border border-gray-100 text-sm text-gray-700">
                            <span x-text="deleteFaqQuestion"></span>
                        </div>
                    </div>
                </div>

                <div class="flex justify-end gap-2 mt-6">
                    <button type="button" @click="deleteFaqId = null"
                        class="h-10 px-4 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50">
                        Batal
                    </button>

                    <form :action="`/admin/faqs/${deleteFaqId}`" method="POST">

                        @csrf
                        @method('DELETE')

                        <button type="submit"
                            class="h-10 px-4 bg-red-600 text-white rounded-xl text-sm hover:bg-red-700">
                            Ya, Hapus
                        </button>
                    </form>
                </div>
