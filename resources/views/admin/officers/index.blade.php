<x-app-layout>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($officers as $officer)
                                <tr class="hover:bg-gray-50/50 transition" x-data="{ editOpen: false }">
                                    ...
                                    <td class="px-5 py-3.5">
                                        <span
                                            class="font-mono text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-lg">
                                            {{ $officer->officer_code }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-3.5 max-w-[150px]">
                                        <p class="text-sm text-gray-700 truncate">{{ $officer->event->title ?? '-' }}
                                        </p>
                                    ...
                                    <td class="px-5 py-3.5">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <form action="{{ route('admin.officers.destroy', $officer) }}"
                                                method="POST" onsubmit="return confirm('Hapus officer ini?')">
                                                @csrf @method('DELETE')
                                                <button type="submit"
                                ...
                                {{-- Inline Edit Row --}}
                                <tr x-show="editOpen" x-transition class="bg-gray-50/50">
        ...
        {{-- Right: Add Form --}}
        {{-- Right: Add Form (Sudah Diperbaiki) --}}
        <div>
            <div class="sticky top-6 space-y-4">
                    ...
                    <form action="{{ route('admin.officers.store') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <select name="user_id"
                                    class="w-full h-10 pl-3 pr-10 border border-gray-100 rounded-xl text-sm text-gray-700 outline-none appearance-none focus:outline-none focus:ring-0 focus:border-gray-300 bg-white @error('user_id')  @enderror">
                        ...
                        <div>
                            ...
                            <div class="relative flex items-center">
                                <select name="event_id"
                                    class="w-full h-10 pl-3 pr-10 border border-gray-100 rounded-xl text-sm text-gray-700 outline-none appearance-none focus:outline-none focus:ring-0 focus:border-gray-300 bg-white @error('event_id')  @enderror">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                @foreach ([['value' => 'scanner', 'label' => 'Scanner', 'icon' => 'ti-scan', 'desc' => 'Scan tiket'], ['value' => 'coordinator', 'label' => 'Coordinator', 'icon' => 'ti-sitemap', 'desc' => 'Koordinasi'], ['value' => 'security', 'label' => 'Security', 'icon' => 'ti-shield', 'desc' => 'Keamanan']] as $roleOpt)
                                    ...
                                    hover:border-gray-300 text-gray-500">
                ...
                {{-- Info Box (Ikut bergeser harmonis saat discroll) --}}
